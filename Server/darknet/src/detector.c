...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh)
{
    ...
    while(1){
        if(filename){
            ...
            if( access( "/home/xukan/Desktop/workspace/yoloserver/agentReady", F_OK ) != -1 ) {
                // file exists
                //printf("Enter Image Path: ");
		//usleep(100);
                fflush(stdout);
                FILE* fp = fopen("/home/xukan/Desktop/workspace/yoloserver/agent.txt","r+");
                if(fp == NULL){
                    fclose(fp);
                    continue;
                }
                input = fgets(input, 256, fp);//input = fgets(input, 256, stdin);
                if(input == NULL){
                    fclose(fp);
                    //remove("/home/xukan/Desktop/workspace/yoloserver/agent.txt");
                    continue;
                }
                //if(!input) return;
                printf("input:%s\n",input);
                ...
                //printf("%s",input);
                fclose(fp);
                remove("/home/xukan/Desktop/workspace/yoloserver/agentReady");
            } else {
                // file doesn't exist
                continue;
            }
