...
import org.tensorflow.demo.tracking.AppendLog;
    ...
    runInBackground(
        new Runnable() {
          ...
          public void run() {
            ...
            if (CameraConnectionFragment.offload){

              Log.d("testtest", "here begin");
            final Bitmap bp = croppedBitmap;
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            bp.compress(Bitmap.CompressFormat.JPEG, 50, baos);
            byte[] b = baos.toByteArray();
            String encodedImage = Base64.encodeToString(b, Base64.DEFAULT);
            //Log.v("testencode",encodedImage);
            //final String encodedImage = ImageBase64.encode(bp);
            HashMap<String, String> hashMap = new HashMap<String, String>();
            hashMap.put("image", encodedImage);
            String response = "";
            //String requestURL = "http://192.168.0.100/upload.php";
              String requestURL = "http://chl.la:65419/upload.php";
            try
            ...
            String temp[] = response.split("uploaded_success");
            ...
            if (temp.length > 1 && temp[1].contains("+")) {
              String[] time = temp[1].split("\\?");
              String[] stuff = time[0].split("#");
              Log.v("PHPProcessingTimeMs", time[1]);
              for (int i = 0; i < stuff.length; i++) {
                Log.v("stuff", "stuff: " + stuff[i]);
                String[] direction = stuff[i].split("\\+");
                int top = Integer.parseInt(direction[0]);
                int bot = Integer.parseInt(direction[1]);
                int left = Integer.parseInt(direction[2]);
                int right = Integer.parseInt(direction[3]);
                String title = direction[4];
                RectF rect = new RectF(left, top, right, bot);
                float prob = Float.parseFloat(direction[5]);
                //final String id, final String title,
                // final Float confidence, final RectF location)
                Classifier.Recognition r = new Classifier.Recognition(null, title, prob, rect);
                results.add(r);
                Log.v("detectorrr", "" + top + " " + bot + " " + title + "  prob: " + prob);
            ...
            Log.v("lastProcessingTimeMs","" + lastProcessingTimeMs);
            AppendLog.Log("lastProcessingTimeMs: " + lastProcessingTimeMs);
            Log.v("imgsizeProcessingTimeMs","" + YOLO_INPUT_SIZE);
            AppendLog.Log("imgsizeProcessingTimeMs: " + YOLO_INPUT_SIZE);
